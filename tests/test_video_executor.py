...
测试 src/executors/video_executor.py 中的视频压制、音频替换、音视频抽取功能。
主要测试参数解析和命令构建的正确性。
"""
import pytest
import os
...
class TestExecuteExtractAV:
    ...
    def test_execute_extract_av_audio_only_aac(self, mock_build, mock_run, mock_video_file):
        """测试仅音频抽取 - AAC 格式"""
        ...
        args.extract_mode = "仅音频"
        
        execute_extract_av(args)
    ...
    @patch('src.executors.video_executor.run_ffmpeg_command')
    @patch('src.executors.video_executor.build_extract_audio_command')
    def test_execute_extract_av_audio_only_mp3(self, mock_build, mock_run, mock_video_file):
        """测试仅音频抽取 - MP3 格式"""
    ...
    @patch('src.executors.video_executor.run_ffmpeg_command')
    @patch('src.executors.video_executor.build_extract_audio_command')
    def test_execute_extract_av_audio_only_wav(self, mock_build, mock_run, mock_video_file):
        """测试仅音频抽取 - WAV 格式"""
    ...
    @patch('src.executors.video_executor.run_ffmpeg_command')
    @patch('src.executors.video_executor.build_extract_audio_command')
    def test_execute_extract_av_audio_only_flac(self, mock_build, mock_run, mock_video_file):
        """测试仅音频抽取 - FLAC 格式"""
    ...
    @patch('src.executors.video_executor.run_ffmpeg_command')
    @patch('src.executors.video_executor.build_extract_video_command')
    def test_execute_extract_av_video_only(self, mock_build_video, mock_run, mock_video_file):
        """测试仅视频抽取模式"""
        mock_build_video.return_value = ["ffmpeg", "-i", mock_video_file, "-an", "-c:v", "copy", "output.mp4"]
        mock_run.return_value = 0
        
        args = MockArgs()
        args.extract_input = mock_video_file
        args.extract_video_output = ""
        args.extract_mode = "仅视频"
        
        execute_extract_av(args)
        
        mock_build_video.assert_called_once()
        call_kwargs = mock_build_video.call_args.kwargs
        assert "_noaudio" in call_kwargs.get("output_path", "")
    
    @patch('src.executors.video_executor.run_ffmpeg_command')
    @patch('src.executors.video_executor.build_extract_video_command')
    @patch('src.executors.video_executor.build_extract_audio_command')
    def test_execute_extract_av_both(self, mock_build_audio, mock_build_video, mock_run, mock_video_file):
        """测试音频+视频同时抽取模式"""
        mock_build_audio.return_value = ["ffmpeg", "-i", mock_video_file, "output.m4a"]
        mock_build_video.return_value = ["ffmpeg", "-i", mock_video_file, "-an", "-c:v", "copy", "output.mp4"]
        mock_run.return_value = 0
        ...
        mock_build_video.assert_called_once()
        # run_ffmpeg_command 应被调用两次 (音频一次，视频一次)
