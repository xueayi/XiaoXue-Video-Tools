...
一个简单的的视频压制与素材素材质量检测工具。基于 Python + Gooey 图形界面，内置 FFmpeg，开箱即用。
...
| **封装转换** | 容器格式转换 (如 MKV → MP4)，不重新编码，支持批量处理 |
| **素材质量检测** | 递归扫描文件夹，检测 PR 不兼容格式、码率/分辨率阈值，输出报告 |
| **音频抽取** | 从视频中提取音频轨道，支持 MP3/AAC/WAV/FLAC 格式 |
| **图片转换** | 批量图片格式互转，支持 PNG/JPG/WEBP/BMP/GIF/TIFF |
| **文件夹创建** | 从 TXT 文件批量创建文件夹，支持自动序号和非法字符处理 |
| **批量重命名** | 批量序列重命名图片/视频，支持递归模式和多种重命名策略 |
...
源代码仓库不包含 FFmpeg 二进制文件，需要手动下载放入 `bin/` 目录，或安装到系统环境变量中。
本项目需要的组件包括ffmpeg和ffprobe。
